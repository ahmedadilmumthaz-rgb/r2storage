...
# --- TOTP 2FA unit checks (RFC 6238 vectors + self-consistency) -----------------
TOTP=backend/dist/auth/totp.js
echo "totp RFC vector T=59:       $(node -e 'const {totpCode}=require(process.argv[1]); process.stdout.write(totpCode("GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ", 59, 8));' "$ROOT/$TOTP")"  # expect 94287082
echo "totp RFC vector T=1.1e9:    $(node -e 'const {totpCode}=require(process.argv[1]); process.stdout.write(totpCode("GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ", 1111111109, 8));' "$ROOT/$TOTP")"  # expect 07081804
echo "totp wrong code rejected:   $(node -e 'const {verifyTotp}=require(process.argv[1]); process.stdout.write(String(verifyTotp("JBSWY3DPEHPK3PXP", "000000", {nowSec: Math.floor(Date.now()/1000)})));' "$ROOT/$TOTP")"  # expect false
echo "totp valid code accepted:   $(node -e 'const {verifyTotp,totpCode}=require(process.argv[1]); const now=Math.floor(Date.now()/1000); const c=totpCode("JBSWY3DPEHPK3PXP", now); process.stdout.write(String(verifyTotp("JBSWY3DPEHPK3PXP", c, {nowSec: now})));' "$ROOT/$TOTP")"  # expect true
echo "totp window accepts prev:   $(node -e 'const {verifyTotp,totpCode}=require(process.argv[1]); const now=Math.floor(Date.now()/1000); const c=totpCode("JBSWY3DPEHPK3PXP", now-30); process.stdout.write(String(verifyTotp("JBSWY3DPEHPK3PXP", c, {nowSec: now})));' "$ROOT/$TOTP")"  # expect true (window=1)
