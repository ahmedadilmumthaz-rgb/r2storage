...
|---|---|
| **Protocol fidelity** | Real AWS **SigV4** signature verification (header + presigned URLs), multipart uploads, `ListObjectsV2`, streaming GET/PUT, and **byte-range + conditional GET** (`Range` → `206`/`416`, `If-None-Match`/`If-Modified-Since` → `304`) — the full stack a media CDN needs. |
...
| **Testing** | Backend smoke suite (**96 checks** — S3, multipart, presigned, ranges/conditionals, auth, rate limits) and a full-platform E2E smoke (`18 checks`), both runnable in CI. |
...
npm test                  # backend smoke suite (96 checks, boots a throwaway instance)
