...
| **Testing** | Backend smoke suite (**331 checks** — S3, multipart, copy, batch delete, object metadata, object tags, listing/subresources (V1+V2), bucket subresources, bucket CORS + preflight, bucket tagging, SSE-C, ListBuckets, lifecycle, presigned, ranges/conditionals, conditional writes, CRC-32 checksums, auth, rate limits) and a full-platform E2E smoke (`18 checks`), both runnable in CI. |
...
npm test                  # backend smoke suite (331 checks, boots a throwaway instance)
