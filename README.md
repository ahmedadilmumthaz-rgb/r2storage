...
|---|---|
| **Protocol fidelity** | Real AWS **SigV4** signature verification (header + presigned URLs, **SDK path-style canonical URIs accepted** — SDKs sign `/bucket/key` against this endpoint, admin URLs sign `/s3/bucket/key`, and **aws-chunked streaming payloads** — `STREAMING-AWS4-HMAC-SHA256-PAYLOAD` chunks verified per-chunk, `STREAMING-UNSIGNED-PAYLOAD`, checksum trailers), multipart uploads (create/part/copy/complete/abort + `ListMultipartUploads`/`ListParts` pagination), server-side **CopyObject** (`x-amz-copy-source`, `x-amz-metadata-directive`), batch **DeleteObjects** (`POST ?delete`, quiet/non-quiet), service-level `ListBuckets` + `ListObjectsV1`+`V2` (`marker` / `continuation-token`, `delimiter`, `encoding-type=url`), **CRC-32 + Content-MD5** integrity checks (`x-amz-checksum-crc32`, `x-amz-checksum-mode`), **bucket lifecycle** (prefix rules, `Days`/`Date` expiration, background sweeper), **bucket CORS** (`PutBucketCors` rule matching on every response + preflight `OPTIONS` for browser uploads) and **bucket tagging**, **SSE-C** (`x-amz-server-side-encryption-customer-*` validate + echo), streaming GET/PUT, **byte-range + conditional GET** (`Range` → `206`/`416`, `If-None-Match`/`If-Modified-Since` → `304`), and **conditional writes** (`If-Match`/`If-Unmodified-Since` → `412`, `If-None-Match: *` → `409`) — the full stack a media CDN needs. |
...
| **Testing** | Backend smoke suite (**347 checks** — S3, multipart, copy, batch delete, object metadata, object tags, listing/subresources (V1+V2), bucket subresources, bucket CORS + preflight, bucket tagging, SSE-C, aws-chunked streaming, ListBuckets, lifecycle, presigned (incl. SDK path-style SigV4), ranges/conditionals, conditional writes, CRC-32 checksums, auth, rate limits) and a full-platform E2E smoke (`18 checks`), both runnable in CI. |
...
npm test                  # backend smoke suite (347 checks, boots a throwaway instance)
